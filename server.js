...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (buf) => {
    ...
    if (type === 'join') {
      const { room = 'default', token } = msg;
      ws.roomToken = token || null;
      joinRoom(ws, room);
    ...
    if (type === 'who') {
      if (!ws.room) return;
      ws.send(JSON.stringify({ type:'room_stats', room: ws.room, count: membersOf(ws.room).length, members: membersOf(ws.room) }));
      return;
    }
    ...
    if (type === 'whisper') {
      ...
      for (const client of set) {
        if (client.id === to && client.readyState === 1) {
          client.send(JSON.stringify({ type: 'message', from: ws.id, payload, at: new Date().toISOString() }));
          break;
        }
      }
      return;
  ...
  ws.on('close', () => {
    ...
    leaveRoom(ws);
    if (wasRoom) {
      broadcast(wasRoom, { type: 'presence', who: ws.id, event: 'leave' });
      pushStats(wasRoom);
    }
  });
});
